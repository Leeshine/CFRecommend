...
public class MRJob{
	...
	public static void main(String[] args) throws Exception{
		...
		Path userVectorDir = new Path("/user/bdpuser19/recommendation/tmp/userVector");
		Path cooccurrenceMatrixDir = new Path("/user/bdpuser19/recommendation/tmp/cooccurrenceMatrix");
		Path resultDir = new Path(args[3]);

		Configuration confUser = new Configuration();
		Job jobUser = new Job(confUser, "userVector");
		jobUser.setJarByClass(MRJob.class);
		...
		jobCo.setJarByClass(MRJob.class);
		...
		jobMatrixM.setJarByClass(MRJob.class);
